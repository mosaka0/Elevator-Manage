...
public class Asansor4 implements Runnable{
    ...
    public void run() {
        while(true){
            ...
            if(kontrol.calisanAsansor[3] == true || gidilecekKat.size()>0){
            synchronized(bilgi.lock1){
                



                    while(gidilecekKat.size()<10 && bilgi.katlar.get(katNo).size()>0 && kontrol.calisanAsansor[3]){
