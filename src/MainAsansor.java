 ...
 * @author ONUR
 */
public class MainAsansor implements Runnable{
    Bilgi bilgi;    
    Kontrol kontrol;
    ...
    public MainAsansor(Bilgi bilgi){
        this.bilgi = bilgi;
    ...
    public void run() {
        ...
        while(true){
            ...
            if(kontrol.calisanAsansor[0] == true){
                ...
                synchronized(bilgi.lock1){
                    ...
                    while(gidilecekKat.size()<10 && bilgi.katlar.get(katNo).size()>0){
                    ...
                    for(int i=0;i<5;i++){
                        boolean br = false;
                        boolean asansifirgidecekkontrol = false;
                        if(i==katNo){
                            continue;
                        }
                        for(int j=0;j<call.size();j++){
    ...
    public void haraket(){
        ...
        //Herhangi bir hedef var ise bu fonksiyona gir
        if(call.size()>0){
                        
            //Gidilecek hedef yukaridaysa yukariya hareket et
            if(call.get(0)>katNo){  
            ...
            //Gidilieck hedef asagidaysa asagiya hareket et
